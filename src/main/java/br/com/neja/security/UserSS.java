...
public class UserSS implements UserDetails{
	...
	public UserSS(Integer id, String email, String senha, Set<Perfil> authorities) {
		...
		this.email = email;
		this.senha = senha;
		this.authorities = authorities.stream().map(x -> new SimpleGrantedAuthority(x.getDescicao())).collect(Collectors.toList());;
	}
	...
	public UserSS(Cliente cli) {
		...
		this.id = cli.getId();
		this.authorities = cli.getPerfis().stream().map(x -> new SimpleGrantedAuthority(x.getDescicao())).collect(Collectors.toList());
	}
